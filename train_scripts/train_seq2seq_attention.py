...
class NMTModel(object):
    def __init__(self):
        self.enc_cell_fw = tf.nn.rnn_cell.MultiRNNCell(
            [tf.nn.rnn_cell.LSTMCell(HIDDEN_SIZE) for _ in range(NUM_LAYERS)])
        self.enc_cell_bw = tf.nn.rnn_cell.MultiRNNCell(
            [tf.nn.rnn_cell.LSTMCell(HIDDEN_SIZE) for _ in range(NUM_LAYERS)])
    ...
    def foward(self, src_input, src_size, trg_input, trg_label, trg_size):
        batch_size = tf.shape(src_input)
        ...
        with tf.variable_scope("encoder"):
            ...
            enc_outputs, enc_state = tf.nn.bidirectional_dynamic_rnn(self.enc_cell_fw, self.enc_cell_bw, \
                                                                     src_emb, src_size, dtype=tf.float32)
...
def main():
    ...
    cost_op, train_op = train_model.foward(src, src_size, trg_input, trg_label, trg_size)

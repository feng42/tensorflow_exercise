...
def run_eval(sess,test_X, test_Y):
    ...
    #rmse = np.sqrt(np.mean(((prediction - labels) ** 2), axis=0))
...
test_start = (TRAINNING_EXAMPLES + TIMESTEPS) * SAMPLE_GAP
test_end = test_start +(TESTING_EXAMPLES + TIMESTEPS) * SAMPLE_GAP
train_X, train_y = generate_data(np.sin(np.linspace(0,test_start,TRAINNING_EXAMPLES+TIMESTEPS,dtype=np.float32)))
